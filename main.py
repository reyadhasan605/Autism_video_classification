...
def get_loaders(opt):
	...
	spatial_transform = Compose([
		...
		# MultiScaleRandomCrop(size=224, scales=(0.5, 1.0)),
		ToTensor(opt.norm_value), norm_method
	])
	temporal_transform = TemporalRandomCrop(40)
	...
	target_transform = ClassLabel()
	temporal_transform = LoopPadding(40)
...
def main_worker():
	...
	for epoch in range(start_epoch, opt.n_epochs + 1):
		...
		if (epoch) % opt.save_interval == 0:
			...
			torch.save(state, os.path.join('/media/zasim/69366298-8e7c-477a-a176-aba06b9848d94/reyad/checkpoints', f'{opt.model}-Epoch-{epoch}-Loss-{val_loss}.pth'))
